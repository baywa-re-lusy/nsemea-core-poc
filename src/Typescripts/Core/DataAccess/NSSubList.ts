...
export class NSSubList <T extends NSSubListLine>{
   ...
   */
  constructor (readonly subListLineType: new (subListId: string, rec: record.Record, line: number) => T,
               nsRec: record.Record, subListId: string) {
  ...
  addLine (ignoreRecalc = true, insertAt: number = this.length): T {
    log.debug('Inserting line', `sublist: ${this._subListId} insert at line:${insertAt}`)
    ...
    return (this._useDynamicModeAPI && this._nsRecord.isDynamic) ? this[this.length] : this[insertAt]
  }
  ...
  removeLine (line: number, ignoreRecalc = false) {
    this._nsRecord.removeLine({ line: line, sublistId: this._subListId, ignoreRecalc: ignoreRecalc })
    this.rebuildArray()
  }
  ...
  protected rebuildArray () {
    ...
    log.debug('Building sublist', `type:${this._subListId}, linecount:${this.length}`)
    ...
    if (this._nsRecord.isDynamic) {
      Object.defineProperty(this, this.length, {
        value: new this.subListLineType(this._subListId, this._nsRecord, this.length),
 ...
 */
function setSublistValue (this: NSSubListLine, fieldId: string, value: FieldValue, isText: boolean) {
    ...
    if (this.useDynamicModeAPI && this._nsRecord.isDynamic) {
      ...
      if (isText) {
        this._nsRecord.setSublistText({ ...options, line: this._line, text: value as string })
      } else {
        this._nsRecord.setSublistValue({ ...options, line: this._line, value: value })
      }
    }
}
 ...
 */
function getSublistValue (this: NSSubListLine, fieldId: string, isText: boolean) {
  const options = {
    sublistId: this._subListId,
    fieldId: fieldId,
  }
  ...
  if (this.useDynamicModeAPI && this._nsRecord.isDynamic) {
    this._nsRecord.selectLine({ sublistId: this._subListId, line: this._line })
    return isText ? this._nsRecord.getCurrentSublistText(options)
      : this._nsRecord.getCurrentSublistValue(options)
  } else {
    return isText ? this._nsRecord.getSublistText({ ...options, line: this._line })
