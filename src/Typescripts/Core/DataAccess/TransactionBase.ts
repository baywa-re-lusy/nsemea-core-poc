...
export class TransactionBase extends NSTypedRecord {
   ...
   */
  protected findApplyLine(
    docId: number,
  ): { apply: boolean; amount: number; line: number } | null {
    const rec = this._nsRecord;
    if (!rec.isDynamic || !this.defaultValues)
      throw new Error(
        'record must be in dynamic mode and have default values set to use this method',
      );
